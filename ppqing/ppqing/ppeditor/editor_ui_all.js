    ...
    function getClipboardData( callback ) {
        ...
        setTimeout( function() {
            if (browser.webkit) {
                ...
                var pastebins = editor.document.querySelectorAll('#baidu_pastebin');
            ...
            domUtils.on(me.body, browser.ie ? 'keydown' : 'paste',function(e){
                ...
                getClipboardData.call( me, function( div ) {
                    ...
                    if ( modify_num.flag && me.ui){
                        me.ui.showToolbarMsg( UEDITOR_CONFIG.messages.pasteMsg );
                        modify_num.flag = "";
                            ...
                            if(li && (pre = li.previousSibling)){
                                ...
                                if(domUtils.isBlockElm(first)){
                                    if(domUtils.isEmptyNode(first)){
                                        ...
                                        range.setStart(first,0).collapse(true).select(true);
    ...
    baidu.editor.plugins['table'] = function() {
       ...
       me.commands['inserttable'] = {
            ...
            execCommand: function (cmdName, tableobj) {
                ...
                //tableobj.border && arr.push( 'border=' + tableobj.border );
                ...
                if (j) while (j --) {
                    ...
                    while (k --) {
                        cols[k] = '<td width='+Math.floor((tableobj.width||500)/tableobj.numCols)+' '+ (tableobj.cellborder || tableobj.bordercolor ? 'style="border:'+(tableobj.cellborder||'1') + 'px solid '+(tableobj.bordercolor||'#000') + '"' : '') + ">"+
                                (browser.ie ? '' : '<br/>') + '</td>';
                    }
                    rows.push('<tr '+(tableobj.align ? 'style=text-align:' + tableobj.align+'' : '')+'>' + cols.join('') + '</tr>');
                }
                
                html = '<table  '+ arr.join(" ")+ ' style="'+(tableobj.backgroundcolor ? 'background-color:' + tableobj.backgroundcolor + ';': '') +
                    (tableobj.cellborder || tableobj.bordercolor ? 'border:'+(tableobj.cellborder||'1') + 'px solid '+(tableobj.bordercolor||'#000') + ';' : '')
