...
<ol id="posts">
    ...
    if ($results = CACHE::get($uid . '_my_' . $page)) {
        ...
        if (!$filter_type) {
            $results = $db->query('select * from ' . $db->getTable(T_CONTENT) . ' where uid=' . $uid . $addtype . ' and sercet=0 order by did desc limit ' . $total . ',' . $sum);
        }
        CACHE::set($uid . '_my_' . $page, $results, 17200);
    }
    if ($filter_type) $results = $db->query('select * from ' . $db->getTable(T_CONTENT) . ' where uid=' . $uid . $addtype . '  and (sercet = NULL or sercet="" or sercet=0) order by did desc limit ' . $total . ',' . $sum);
    ...
    <?php }?>
       ...
       <div class="so_ie_doesnt_treat_this_as_inline">
           ...
           <a class="post_avatar" id="post_controls_avatar"
              style="background-image:url('<?php echo BASE_URL . 'css/fan_skin/bgimg/type-icons.png'?>'); background-position: -<?php echo ($value['types'] - 1) * 75;?> 0"
              title="" href="index.php?m=ta&uid=<?php echo $value['uid'];?>&filter_type=<?php echo $value['types'];?>">
               &nbsp;</a>
       </div>
 </li>
<?php }?>
