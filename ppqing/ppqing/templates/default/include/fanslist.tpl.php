...
$total = ($page - 1) * $sum;
$db = APP :: ADP('db');
$rows = $db->query('select count(uid) as count from ' . $db->getTable(T_FOLLOWS) . '  where guid=' . $userinfo['uid']);
$allcount = $rows[0]['count'];
$fresults = $db->query('select *,x.uid as uid,y.name as name,z.uid as zuid  from ' . $db->getTable(T_FOLLOWS) . '  x left join ' . $db->getTable(T_USERS) . ' y on x.uid=y.uid   left join  ' . $db->getTable(T_FOLLOWS) . '  z on y.uid=z.guid and z.uid=' . $userinfo['uid'] . ' where x.guid=' . $userinfo['uid'] . ' order by x.fid desc limit ' . $total . ',' . $sum);
?>
<style type="text/css">
    <!--
    body {
...
<div class='clist'>
    <div class='tagtitle'>
        <span style='font-size:18px;font-weight:bold;color:'>慕名者有（<?php echo $allcount;?>）人</span>
    </div>
...
<?php } ?>
<?php $theurl = "index.php?m=index.fans";
